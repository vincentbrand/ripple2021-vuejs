    ...
    >
      ...
      <div v-if="r.children">
        ...
        >
          ...
          <span><!-- {{ $t('interface.sidemenu.'+r.name) }}--> {{ r.name }}</span>
        </a>
        ...
        >
          <div class="bg-white py-2 collapse-inner rounded" v-if="r.children.length > 0">
            <router-link
            ...
            >
              ...
              <!-- {{ $t('interface.sidemenu.'+rch.name) }} --> {{ rch.name }}
            </router-link>
          </div>
        </div>
      </div>
      ...
      <div v-else :class="r.path ? '' : 'sidebar-heading font-weight-bold'">
        <span v-if="!r.path"><!--{{ $t('interface.sidemenu.'+rch.name) }} --> {{ rch.name }}</span>
        <router-link class="nav-link" :to="r.path" v-else>
          <i :class="'fas fa-fw ' + r.icon"></i>
          <span><!-- {{ $t('interface.sidemenu.'+r.name) }} --> {{ rch.name }}</span>
        </router-link>
      </div>
    </li>
